...
const Signup: React.FC = () => {
  ...
  const [newUser, setNewUser] = useState<SignupData>({nickname: null , userid: null, password: null})
  ...
  useEffect(() => {
    const validation = async () => {
      ...
      try {
        ...
        res.status === 200 ? addNewUser() : null
      } catch (error: any) {
  ...
  const children: React.ReactNode = (
    <>
      ...
      <ToastContainer position='bottom-center' limit={2} closeButton={false} autoClose={3000} hideProgressBar/>
    </>
  );
  return (
    <>
